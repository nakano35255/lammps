...
#include "compute_hexorder_atom_mod.h"
#include <cmath>
#include <cstring>
#include <complex>
#include "atom.h"
...
ComputeHexOrderAtomMod::ComputeHexOrderAtomMod(LAMMPS *lmp, int narg, char **arg) :
  Compute(lmp, narg, arg),
  distsq(nullptr), nearest(nullptr), qnarray(nullptr)
{
  if (narg < 3 ) error->all(FLERR,"Illegal compute hexorder/atom/mod command");
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg],"degree") == 0) {
      if (iarg+2 > narg) error->all(FLERR,"Illegal compute hexorder/atom/mod command");
...
ComputeHexOrderAtomMod::~ComputeHexOrderAtomMod()
{
...
void ComputeHexOrderAtomMod::init()
{
  if (force->pair == nullptr)
    error->all(FLERR,"Compute hexorder/atom/mod requires a pair style be defined");
  if (cutsq == 0.0) cutsq = force->pair->cutforce * force->pair->cutforce;
  else if (sqrt(cutsq) > force->pair->cutforce)
    error->all(FLERR,
               "Compute hexorder/atom/mod cutoff is longer than pairwise cutoff");
  ...
  for (int i = 0; i < modify->ncompute; i++)
    if (strcmp(modify->compute[i]->style,"hexorder/atom/mod") == 0) count++;
  if (count > 1 && comm->me == 0)
    error->warning(FLERR,"More than one compute hexorder/atom/mod");
}

/* ---------------------------------------------------------------------- */

void ComputeHexOrderAtomMod::init_list(int /*id*/, NeighList *ptr)
{
  list = ptr;
}
...
void ComputeHexOrderAtomMod::compute_peratom()
{
  ...
  if (atom->nmax > nmax) {
    memory->destroy(qnarray);
    nmax = atom->nmax;
    memory->create(qnarray,nmax,ncol,"hexorder/atom/mod:qnarray");
    array_atom = qnarray;
  }
  ...
  inum = list->inum; //number of atoms for which neighbor lists exist
  ilist = list->ilist; //list of "i" atoms for which neighbor lists exist
  numneigh = list->numneigh; //length of each neighbor list
  firstneigh = list->firstneigh; //pointrer to the list of neighbors of "i"
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    if ((mask[i] & groupbit) && (zlo < x[i][2]) && (x[i][2] < zhi)) {
      ...
      for (jj = 0; jj < jnum; jj++) {
        j = jlist[jj];
        j &= NEIGHMASK; // remove? "special pair" i.e. part of a topological pair, angle or dihedral
...
inline void ComputeHexOrderAtomMod::calc_qn_complex(double delx, double dely, double &u, double &v) {
...
inline void ComputeHexOrderAtomMod::calc_qn_trig(double delx, double dely, double &u, double &v) {
  double ntheta;
...
void ComputeHexOrderAtomMod::select2(int k, int n, double *arr, int *iarr)
{
...
double ComputeHexOrderAtomMod::memory_usage()
{
